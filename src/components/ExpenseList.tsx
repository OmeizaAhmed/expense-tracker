...
export default function ExpenseList({
  ...
  const { totalTransaction, setTotalTransaction }: any =
    useContext(GlobalContext);
  const expenseData: formDataType[] = totalTransaction.filter(
    (item: formDataType) => item.type === title
  );

  function removeFromExpense(id: number) {
    const cpyTotalTransaction = [...totalTransaction];
    const transactionIndex = cpyTotalTransaction.findIndex(
    ...
    <div className="flex flex-col gap-3 bg-white p-5 rounded-2xl">
      ...
      {expenseData && expenseData.length
        ? expenseData.map((item) => (
            <div
              key={item.id}
              className={`w-full rounded-lg py-3 px-6 pr-9 flex relative justify-between capitalize ${
                title === "income" ? "bg-blue-100" : "bg-red-100"
              }`}
            >
              ...
              <span
                className="absolute top-1.5 right-3 text-xs font-mono cursor-pointer font-bold"
                onClick={() => (item.id ? removeFromExpense(item.id) : null)}
              >
                x
              </span>
            </div>
